...
async function processScrapeWorkflow() {
  try {
    console.log("🔄 [WORKFLOW PART 1] Fetching existing WordPress articles...");
    ...
    console.log(`✅ [WORKFLOW PART 1] Found ${wpArticles.length} existing WordPress articles`);
    ...
    for await (const batch of scrapeWikiVenderFlow()) {
      console.log("📦 [WORKFLOW PART 1] Processing a new batch with", batch.length, "categories.");
      ...
      console.log("✅ [WORKFLOW PART 1] Batch processed:", {
        updates: articleneedtoupdateBatch.length,
        newArticles: newDataForWpBatch.length,
      });
    }

    console.log("\n🎉 [WORKFLOW PART 1] Scraping complete!");
    console.log(`📊 [WORKFLOW PART 1] Summary:`);
    console.log(`   - Articles to update: ${aggregatedArticlesToUpdate.length}`);
    console.log(`   - New articles to create: ${aggregatedNewDataForWp.length}`);
    ...
    console.error("❌ [WORKFLOW PART 1] Error in processScrapeWorkflow:", err);
...
async function processScrapeWorkflowconvotab() {
  try {
    console.log("🔄 [WORKFLOW PART 2] Fetching existing WordPress articles...");
    // Fetch WP articles once (assumed to be a small list)
    const wpResponse = await axios.get(
      "https://wiki.venderflow.com/wp-json/custom/v1/get-hkbs",
      {
    ...
    const wpArticles = wpResponse.data;
    console.log(`✅ [WORKFLOW PART 2] Found ${wpArticles.length} existing WordPress articles`);
    ...
    for await (const batch of scrapeWikiVenderFlowFromConversations()) {
      console.log("📦 [WORKFLOW PART 2] Processing a new batch with", batch.length, "categories.");
      ...
      console.log("✅ [WORKFLOW PART 2] Batch processed:", {
        updates: articleneedtoupdateBatch.length,
        newArticles: newDataForWpBatch.length,
      });
    }

    console.log("\n🎉 [WORKFLOW PART 2] Scraping complete!");
    console.log(`📊 [WORKFLOW PART 2] Summary:`);
    console.log(`   - Articles to update: ${aggregatedArticlesToUpdate.length}`);
    console.log(`   - New articles to create: ${aggregatedNewDataForWp.length}`);
    ...
    console.error("❌ [WORKFLOW PART 2] Error in processScrapeWorkflow:", err);
    throw new Error(err.message);
  }
}
